...
const PerfumePromoBanner = require('../models/perfumePromoBannerModel');
...
const getOrCreate = async () => {
    let doc = await PerfumePromoBanner.findOne();
    if (!doc) doc = await PerfumePromoBanner.create({});
    return doc;
};

exports.getPublicPerfumePromoBanner = asyncErrorHandler(async (req, res) => {
...
exports.getAdminPerfumePromoBanner = asyncErrorHandler(async (req, res) => {
    const doc = await getOrCreate();
...
exports.upsertAdminPerfumePromoBanner = asyncErrorHandler(async (req, res, next) => {
    const doc = await getOrCreate();
    ...
    const maxBytes = Number(process.env.PERFUME_PROMO_BANNER_IMAGE_MAX_BYTES || 8_000_000);

    const imageFile = req?.files?.image;
    if (imageFile) {
        const dataUrl = await readImageFileToDataUrl(imageFile, { maxBytes, fieldName: 'Perfume banner' }, next);
        if (!dataUrl) return;

        const uploaded = await uploadToCloudinaryOrFallback(
            {
                data: dataUrl,
                folder: 'perfume-promo-banner',
                fallbackPrefix: 'perfume-promo',
            },
            next
        );
...
exports.clearAdminPerfumePromoBanner = asyncErrorHandler(async (req, res) => {
    const doc = await getOrCreate();
