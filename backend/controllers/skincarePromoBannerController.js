...
const SkinCarePromoBanner = require('../models/skincarePromoBannerModel');
...
const getOrCreate = async () => {
    let doc = await SkinCarePromoBanner.findOne();
    if (!doc) doc = await SkinCarePromoBanner.create({});
    return doc;
};

exports.getPublicSkinCarePromoBanner = asyncErrorHandler(async (req, res) => {
...
exports.getAdminSkinCarePromoBanner = asyncErrorHandler(async (req, res) => {
    const doc = await getOrCreate();
...
exports.upsertAdminSkinCarePromoBanner = asyncErrorHandler(async (req, res, next) => {
    const doc = await getOrCreate();
    ...
    const maxBytes = Number(process.env.SKINCARE_PROMO_BANNER_IMAGE_MAX_BYTES || 8_000_000);

    const imageFile = req?.files?.image;
    if (imageFile) {
        const dataUrl = await readImageFileToDataUrl(imageFile, { maxBytes, fieldName: 'SkinCare banner' }, next);
        if (!dataUrl) return;

        const uploaded = await uploadToCloudinaryOrFallback(
            {
                data: dataUrl,
                folder: 'skincare-promo-banner',
                fallbackPrefix: 'skincare-promo',
            },
            next
        );
...
exports.clearAdminSkinCarePromoBanner = asyncErrorHandler(async (req, res) => {
    const doc = await getOrCreate();
